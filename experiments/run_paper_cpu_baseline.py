...
def main() -> None:
    parser = base_arg_parser("Run the cpu_original paper-style baseline.")
    ...
    out = REPO_ROOT / "results" / "paper_reproduction" / "cpu_original" / "runs.csv"
    ...
    for dataset in datasets:
        for seed in args.seeds:
            for height in tree_heights:
                ...
                write_run_outputs(
                    result=result,
                    run_id=f"cpu_original_{dataset}_{seed}_h{height}",
                    dataset=dataset,
                    backend="cpu_original",
                    seed=seed,
                    tree_height=height,
