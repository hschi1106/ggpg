...
GPU GP systems commonly exploit data-parallel fitness evaluation using
linearized programs or interpreters~\cite{langdon2008simd}. Our workload is
finer-grained than conventional generational GP: candidate programs are small,
meaningful candidates appear throughout GOM, and exact accept--reject decisions
are sequential. The core question is therefore not simply ``can fitness run on a
GPU?'' but where exact GPU evaluation becomes worthwhile, and how much search
distortion is traded for throughput when candidate evaluations are batched.
...
The exact path is launch-bound, so it uses CUDA Graph replay for the repeated
copy--clear--kernel--copy sequence, pinned host buffers, capacity-managed device
buffers, and single-precision device arrays. The batch path uses the same token
format but increases program-level parallelism by evaluating up to the requested
batch size of meaningful candidates per launch. Host-side subtree vectors and
serialization buffers are cached to reduce per-candidate allocation. A persistent
kernel was tested during development but rejected because queue management and
synchronization overhead outweighed launch savings for this workload.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{fitness_evolution_by_generation.png}
...
The profiling result of 334 programs per launch for batch GPU versus roughly one
program per launch for exact GPU is the most direct evidence.
