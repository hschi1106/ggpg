...
namespace g {
  ...
  ExecutionBackend execution_backend = ExecutionBackend::CPU_ORIGINAL;
  int gpu_batch_size = 32;
  bool gpu_check_correctness = false;
  GpuEvalContext * gpu_ctx = NULL;
  long long num_gpu_evaluations = 0;
  long long num_cpu_evaluations = 0;
  long long num_accepted_moves = 0;
  ...
  ExecutionBackend parse_execution_backend(string setting) {
    if (setting == "cpu_original") {
      return ExecutionBackend::CPU_ORIGINAL;
    } else if (setting == "gpu_exact_gom") {
      return ExecutionBackend::GPU_EXACT_GOM;
    } else if (setting == "gpu_batch_gom") {
      return ExecutionBackend::GPU_BATCH_GOM;
    } else {
      throw runtime_error("Unrecognized execution backend: "+setting);
    }
  }

  string str_execution_backend() {
    if (execution_backend == ExecutionBackend::CPU_ORIGINAL) {
      return "cpu_original";
    }
    if (execution_backend == ExecutionBackend::GPU_EXACT_GOM) {
      return "gpu_exact_gom";
    }
    return "gpu_batch_gom";
  }

  bool using_gpu_backend() {
    return execution_backend != ExecutionBackend::CPU_ORIGINAL;
  }

  GpuFitnessKind current_gpu_fitness_kind() {
    if (!fit_func) {
      throw runtime_error("Cannot configure GPU fitness before fitness function is set");
    }
    string fit_name = fit_func->name();
    if (fit_name == "mse") {
      return GpuFitnessKind::MSE;
    }
    if (fit_name == "mae") {
      return GpuFitnessKind::MAE;
    }
    throw runtime_error("GPU backends currently support fitness_function=mse or mae. Got: "+fit_name);
  }

  void ensure_cuda_backend_available() {
#ifndef GPG_USE_CUDA
  ...
  void sync_gpu_context() {
    if (!using_gpu_backend()) {
      return;
    }
    ensure_cuda_backend_available();
    if (!fit_func) {
      throw runtime_error("Cannot initialize GPU context before fitness function is set");
    }
    if (fit_func->X_batch.rows() == 0 || fit_func->y_batch.rows() == 0) {
      throw runtime_error("Cannot initialize GPU context before the training batch is set");
    }
    if (!gpu_ctx) {
      gpu_ctx = new GpuEvalContext();
    }

    if (gpu_ctx->initialized) {
      gpu_eval_set_data(*gpu_ctx, fit_func->X_batch, fit_func->y_batch);
    } else {
      gpu_eval_init(*gpu_ctx, fit_func->X_batch, fit_func->y_batch, 64, max(1, gpu_batch_size));
    }
  ...
  void reset() {
    ...
    num_gpu_evaluations = 0;
    num_cpu_evaluations = 0;
  ...
  void read_options(int argc, char** argv) {
    ...
    parser.set_optional<int>("ims_g", "ims_sub_generations", 4, "IMS sub-generations before spawning the next population");
    // initialization
    ...
    parser.set_optional<string>("backend", "execution_backend", "cpu_original", "Execution backend: cpu_original, gpu_exact_gom, gpu_batch_gom");
    parser.set_optional<int>("gpu_batch_size", "gpu_batch_size", 32, "Candidate batch size for gpu_batch_gom");
    parser.set_optional<bool>("gpu_check_correctness", "gpu_check_correctness", false, "Check sampled CPU/GPU fitness agreement");
    // other
    ...
    ims_sub_generations = max(1, parser.get<int>("ims_g"));
    if (disable_ims) {
      pop_size = parser.get<int>("pop");
      print("pop. size: ",pop_size);
    } else {
      pop_size = 64;
      print("IMS active, sub-generations: ", ims_sub_generations);
    }
    ...
    execution_backend = parse_execution_backend(parser.get<string>("backend"));
    gpu_batch_size = max(1, parser.get<int>("gpu_batch_size"));
    gpu_check_correctness = parser.get<bool>("gpu_check_correctness");
    ensure_cuda_backend_available();
    print("execution backend: ", str_execution_backend(), ", gpu batch size: ", gpu_batch_size, ", gpu correctness checks: ", gpu_check_correctness);
    // problem
