...
float evaluate_node_gpu(Node * tree) {
  if (!g::gpu_ctx) {
    throw runtime_error("GPU context is not initialized");
  }

  SerializedProgram sp = serialize_active_tree_to_postfix(tree);
  ...
  if (g::gpu_check_correctness) {
    ...
    float relative_tolerance = max(abs(cpu_fitness), abs(fitness)) > (float) 1e6 ? (float) 1e-2 : (float) 1e-3;
...
Node * efficient_gom_gpu_exact(Node * parent, vector<Node*> & population, vector<vector<int>> & fos, GpuEvalContext & gpu_ctx) {
  (void) gpu_ctx;
  Node * offspring = parent->clone();
  float backup_fitness = parent->fitness;
  vector<Node*> offspring_nodes = offspring->subtree();
  ...
  for(int fos_idx = 0; fos_idx < fos.size(); fos_idx++) {
    ...
    if (change_is_meaningful) {
      ...
      new_fitness = evaluate_node_gpu(offspring);
    } else {
...
Node * efficient_gom(Node * parent, vector<Node*> & population, vector<vector<int>> & fos) {
  if (g::execution_backend == g::ExecutionBackend::GPU_EXACT_GOM) {
    return efficient_gom_gpu_exact(parent, population, fos, *g::gpu_ctx);
  }
...
void evaluate_nodes_gpu_batch(vector<Node*> & nodes, vector<float> & out_fitness) {
  ...
  vector<SerializedProgram> serialized;
  serialized.reserve(batch_size);
  int max_program_len = 1;
  for(Node * n : nodes) {
    serialized.push_back(serialize_active_tree_to_postfix(n));
    max_program_len = max(max_program_len, (int) serialized.back().tokens.size());
  }

  vector<GpuToken> flat_programs((size_t) batch_size * (size_t) max_program_len);
  vector<int> lengths(batch_size);
  for(int i = 0; i < batch_size; i++) {
    lengths[i] = (int) serialized[i].tokens.size();
    for(int j = 0; j < lengths[i]; j++) {
      flat_programs[(size_t) i * (size_t) max_program_len + (size_t) j] = serialized[i].tokens[j];
    }
  }

  vector<double> gpu_fitness(batch_size);
  ...
  for(int i = 0; i < batch_size; i++) {
    ...
    float fitness = (float) gpu_fitness[i];
    if (!isfinite(fitness) || fitness < 0) {
      fitness = INF;
    }
    ...
    if (g::gpu_check_correctness) {
      double gpu_single = evaluate_fitness_gpu_single(
        *g::gpu_ctx,
        serialized[i].tokens.data(),
        (int) serialized[i].tokens.size()
      );
      double gpu_tolerance = max(1e-6, abs(gpu_single) * 1e-8);
      if (isfinite(gpu_single) && isfinite(gpu_fitness[i]) && abs(gpu_single - gpu_fitness[i]) > gpu_tolerance) {
        throw runtime_error(
          "GPU batch/single fitness mismatch: single=" + to_string(gpu_single) +
          ", batch=" + to_string(gpu_fitness[i])
        );
      }

      float cpu_fitness = g::fit_func->get_fitness(nodes[i], g::fit_func->X_batch, g::fit_func->y_batch);
      float tolerance = max((float) 1e-4, abs(cpu_fitness) * (float) 1e-3);
...
void gomea_generation_gpu_batch(vector<Node*> & population, vector<vector<int>> & fos, GpuEvalContext & gpu_ctx, int pop_batch_size) {
  ...
  for(int batch_start = 0; batch_start < order.size(); batch_start += pop_batch_size) {
    ...
    for(int step = 0; step < fos.size(); step++) {
      ...
      for(int i = 0; i < B; i++) {
        auto crossover_mask = fos[fos_orders[i][step]];
        bool change_is_meaningful = false;
      ...
      evaluate_nodes_gpu_batch(eval_nodes, new_fitnesses);
